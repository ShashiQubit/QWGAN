...
# Show the plot
plt.savefig("plots/S&P 500 Index Direct Returns_and_S&P 500 Index Log Returns.svg", format="svg")
plt.close()
...
plt.ylabel(r'$\rho$')
plt.grid()
plt.savefig("plots/ACF S&P 500 Log-Returns.svg", format="svg")
plt.close()
...
plt.grid()
plt.savefig("plots/ACF S&P 500 Absolute Log-Returns.svg", format="svg")
plt.close()
...
plt.ylabel(r'$L(\tau)$')
plt.grid()
plt.savefig("plots/Leverage Effect S&P 500.svg", format="svg")
plt.close()
...
plt.grid(True)
plt.savefig("plots/Q-Q Plot - S&P 500 Log Returns.svg", format="svg")
plt.close()
...
plt.ylabel('Sample Quantiles')
plt.grid(True)
plt.savefig("plots/Q-Q Plot - Transformed S&P 500 Log Returns.svg", format="svg")
plt.close()
    ...
    def train_qgan(self, gan_data, original_data, preprocessed_data, num_elements):
        ...
        for epoch in range(self.num_epochs):
            ...
            if epoch % 100 == 0 or epoch+1 == self.num_epochs:
                ...
                print(f'Critic loss (average): {self.critic_loss_avg[-1][0]}')
                print(f'Generator loss (average): {self.generator_loss_avg[-1]}')
                print(f'\nEMD (average): {self.emd_avg[-1]}')
                print(f'ACF RMSE (average): {self.acf_avg[-1]}')
                print(f'VOLATILITY RMSE (average): {self.vol_avg[-1]}')
                print(f'LEVERAGE RMSE (average): {self.lev_avg[-1]}\n')
...
qgan.train_qgan(gan_data, sp500_log_r_tf, transformed_sp500, num_elements)
